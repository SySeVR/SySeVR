...
def _standardize_input_data(data, names, shapes=None,
    ...
    if isinstance(data, dict):
        ...
        #change to adapt use test batch == 1 - Yoki
        if type(data[0]) == int:
            if data == [0] or data == [1]:
                data = [np.asarray(data)]
    ...
    def evaluate_generator(self, generator, steps=None,
        ...
        try:
            ...
            while steps_done < steps:
                ...
                #适用于merics的列表中有对应的TP/FP/FN
                if outs[1] == 1:
                    TP_index.append(steps_done)
                if outs[2] == 1:
                    FP_index.append(steps_done)
                if outs[3] == 1:
                    FN_index.append(steps_done)
                if outs[1] == 0 and outs[2] == 0 and outs[3] == 0:
                    TN_index.append(steps_done)
            ...
            if enqueuer is not None:
                enqueuer.stop()
            '''
            f = open("FPFN_index.txt","a")
            f.write("--------------------")
            f.write("FP_index"+str(FP_index)+"\n")
            f.write("FN_index"+str(FN_index))
            f.close()
            '''
        if not isinstance(outs, list):
            ...
            f.close()
            import pickle
            f = open('TPFPFN_count.pkl', 'wb')
            '''
            x = []
            for i in range(len(outs)):
                if i == 1 or i == 2 or i == 3:
                    #对于计算count只记录总数
                    averages.append(np.sum([out[i] for out in all_outs]))
                    x.append([out[i] for out in all_outs])
                else:
                    #对于数值类取平均
                    averages.append(np.average([out[i] for out in all_outs],
                                           weights=batch_sizes))

            return [averages,TP_index,FP_index,FN_index,TN_index]
